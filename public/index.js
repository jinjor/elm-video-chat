...
Elm.Lib = Elm.Lib || {};
Elm.Lib.URI = Elm.Lib.URI || {};
Elm.Lib.URI.make = function (_elm) {
   "use strict";
   _elm.Lib = _elm.Lib || {};
   _elm.Lib.URI = _elm.Lib.URI || {};
   if (_elm.Lib.URI.values)
   return _elm.Lib.URI.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
...
Elm.Main.make = function (_elm) {
   ...
   var peerView = function (peer) {
      return A2($Html.li,
      _L.fromArray([]),
      _L.fromArray([$Html.text(peer.name)]));
   };
   var userOf = F2(function (d,
   peerId) {
      return function () {
         var _v0 = A2($Dict.get,
         peerId,
   ...
   var roomView = function (room) {
      return function () {
         ...
         room.id))]),
         _L.fromArray([A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("room-name panel-title")]),
         _L.fromArray([$Html.text(room.id)]))]))]));
         var usersDict = $Dict.fromList(room.users);
         var users = A2($List.map,
         function (peerId) {
            return A2(userOf,
            usersDict,
            peerId);
      ...
      return function () {
         ...
         {case "Init":
            return _U.replace([["me"
                               ,action._0.user]
                              ,["rooms",action._0.rooms]],
              context);
            case "UpdateRoomName":
            return _U.replace([["roomName"
                               ,action._0]],
              context);}
         _U.badCase($moduleName,
   ...
   var UpdateRoomName = function (a) {
      return {ctor: "UpdateRoomName"
             ,_0: a};
   };
   var createRoomView = F2(function (address,
   c) {
      return function () {
         var submit_ = A2($Html.input,
         _L.fromArray([$Html$Attributes.type$("submit")
                      ,$Html$Attributes.$class("btn btn-primary")
                      ,$Html$Attributes.value("Create")]),
         _L.fromArray([]));
         var input_ = A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("form-group")]),
         _L.fromArray([A2($Html.label,
                      _L.fromArray([]),
                      _L.fromArray([$Html.text("New Room")]))
                      ,A2($Html.input,
                      _L.fromArray([$Html$Attributes.name("")
                                   ,$Html$Attributes.$class("form-control")
                                   ,$Html$Attributes.value(c.roomName)
                                   ,A3($Html$Events.on,
                                   "input",
                                   $Html$Events.targetValue,
                                   function ($) {
                                      return $Signal.message(address)(UpdateRoomName($));
                                   })]),
                      _L.fromArray([]))]));
         var form_ = A2($Html.form,
         _L.fromArray([$Html$Attributes.$class("form-inline")
                      ,$Html$Attributes.action(A2($Basics._op["++"],
                      "/room/",
                      $Lib$URI.encodeURI(c.roomName)))
                      ,$Html$Attributes.method("GET")]),
         _L.fromArray([input_,submit_]));
         return A2($Html.div,
         _L.fromArray([]),
         _L.fromArray([form_]));
      }();
   ...
   c) {
      ...
      _L.fromArray([$Lib$Header.header({_: {}
                   ...
                   _L.fromArray([$Html$Attributes.$class("container")]),
                   _L.fromArray([A2($Html.ul,
                                _L.fromArray([$Html$Attributes.$class("list-unstyled clearfix col-md-12")]),
                                roomViews(c))
                                ,A2(createRoomView,
                                address,
                                c)]))]));
   ...
   var fetchRoom = Elm.Native.Task.make(_elm).perform(A2($Task.onError,
   A2($Task.andThen,
   $Lib$API.getRooms,
   function (initData) {
